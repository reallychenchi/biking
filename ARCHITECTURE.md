...
4. 点击历史卡片或“我的”个人纪录进入详情页前，入口页先隐藏外层骑行/历史/我的 TabBar，再打开详情导航；详情页通过 `.task(id:)` 调用 `completedRide(id:)`，该查询使用专用 `FetchDescriptor` 并设置 `relationshipKeyPathsForPrefetching = [\RideEntity.points]` 预取轨迹关系；加载期间显示进度和禁用的详情/轨迹切换栏，成功后才展示详情指标、速度图、轨迹页、分享按钮并允许切换，失败提供重试入口（`iOS/bike/bike/Features/History/HistoryView.swift`、`iOS/bike/bike/Features/Settings/SettingsView.swift`、`iOS/bike/bike/Features/History/RideDetailView.swift`、`iOS/bike/bike/Persistence/SwiftDataRideRepository.swift`）。
